...
import type { CmsEntry, CmsSocialLink } from "@/lib/emdash-content";
import { ExternalLink } from "lucide-react";

interface LinkTreeProps {
  links?: CmsEntry<CmsSocialLink>[];
}

export default function Component({ links: cmsLinks = [] }: LinkTreeProps) {
  ...
  const links = cmsLinks.length
    ? cmsLinks.map((link) => ({
        icon: <SocialIcon platform={link.data.platform} title={link.data.title} />,
        title: link.data.title,
        subtitle: link.data.subtitle,
        description: link.data.description,
        url: link.data.url,
        button: link.data.button_text
          ? { text: link.data.button_text, icon: link.data.button_icon }
          : undefined,
        className: link.data.class_name,
      }))
    : fallbackLinks;
    ...
    <div className="container mx-auto p-4">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {links.map((link, index) => (
          <Card
            key={index}
            className={`overflow-hidden transition-transform transform hover:scale-[1.02] ${
              link.className || ""
            }`}
          >
              ...
              <div className="flex h-full flex-col gap-3">
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-grow">
                    <h2 className="break-words text-lg font-semibold leading-tight">{link.title}</h2>
                    {link.subtitle && (
...
const SvgIcon: React.FC<SvgIconProps> = ({ title, path, className }) => (
  <svg
    role="img"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
...
const SocialIcon = ({ platform, title }: { platform: string; title: string }) => (
  <SvgIcon title={title} path={iconPaths[platform] ?? iconPaths.telegram} className="w-8 h-8" />
);
